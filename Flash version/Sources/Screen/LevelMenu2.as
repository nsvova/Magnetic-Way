...
//	import flash.display.NativeMenu;
	...
	public class LevelMenu2 extends GameScreen
	{
		...
		private var lockScreen:Sprite = null;
		private var yourBonus:TextField = null;
		private var haveToBeBonus:TextField = null;
		
		
		private var tempButton:String;
		
		public function LevelMenu2():void
		{
		...
		override public function Start():void
		{
			...
			var allBonus:int = Bonus.AllBonus();
			
			if (allBonus >= 25)
			{
				listLevels = FlashGrafic.drawManyLevelBoxs(10, 170, 170, 50, 50, 25, 5, 11);
			...
			{
				lockScreen = new gLockLevels();
				lockScreen.x = 50;
				lockScreen.y = 30;
				addChild(lockScreen);
				
				yourBonus = FlashGrafic.CreateText(allBonus.toString(),
													240, 278, 32, 
													0xffffff, "Comic Sans MS", 50);
													
				haveToBeBonus = FlashGrafic.CreateText("25",
													338, 278, 32, 
													0xffffff, "Comic Sans MS", 50);
													
				addChild(yourBonus);
				addChild(haveToBeBonus);
			}
			...
			_prev.addEventListener(MouseEvent.CLICK, clickPrev);
		...
		{
			var timer:Timer = new Timer(500, 1);
			timer.addEventListener(TimerEvent.TIMER_COMPLETE, timerClickPrev);
		...
		private function timerClickPrev(e:TimerEvent):void 
		{
		...
		override public function Stop():void
		{
			if (lockScreen)
			{
				removeChild(lockScreen);
				lockScreen = null;
				
				if (yourBonus)
				{
					removeChild(yourBonus);
					yourBonus = null;
				}
				if (haveToBeBonus)
				{
					removeChild(haveToBeBonus);
					haveToBeBonus = null;
				}
			}
